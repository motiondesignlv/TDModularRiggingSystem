...
class TDFootRigging(System.ModularRiggingSystem):
    def __init__(self):
        super(TDFootRigging,self).__init__()
        #self  = System.ModularRiggingSystem()
    ...
    def createFootJoint(self,footJoints):
        self.footJoint = self.createRiggingJoint(footJoints,1,"Add")
    ...
    def createFootFKJoint(self,footJoints):
        self.FKJoint = self.createRiggingJoint(footJoints,0,"FK")#FKジョイントを取得
        cmds.setAttr(self.FKJoint[1]+".preferredAngleX",90)
        self.FKFootGP = self.createGP(self.FKJoint[0],"%s_Grp"%self.FKJoint[2])
    ...
    def createFootIKJoint(self,footJoints):
        self.IKJoint = self.createRiggingJoint(footJoints,0,"IK")#FKジョイントを取得
        cmds.setAttr(self.IKJoint[1]+".preferredAngleX",90)
        self.IKFootGP = self.createGP(self.IKJoint[0],"%s_Grp"%self.IKJoint[2])
    ...
    def createFootFKCtl(self,FKJoint,CtlColor,CtlScale):
        ...
        for fkFootCtls in range(len(FKJoint)):
            self.FKFootCtl = self.createRigController(self.TDcrc.TDCircle,FKJoint[fkFootCtls],FKJoint[fkFootCtls],CtlColor,15*CtlScale)
            if fkFootCtls > 0:
        ...
        self.FKFootCtlGP = self.createGP(self.FKFootCtlOffset[0],"%s_Grp"%self.FKFootCtls[0])

        return [self.FKFootCtlGP, self.FKFootCtls[-1]]

    #IK制御用のコントローラーを作成
    def createFootIKCtl(self,IKjoint,CtlColor,CtlScale):
        self.IKFootCtr = self.TDcrc.TDSquare("%s_Ctl"%IKjoint)
        cmds.setAttr(self.IKFootCtr+".overrideEnabled",1)
        cmds.setAttr(self.IKFootCtr+".overrideColor",CtlColor)
        self.setNurbsCtlScaling(self.IKFootCtr,10*CtlScale,15*CtlScale,15*CtlScale)
        self.IKFootCtrOffset = self.createGP(self.IKFootCtr,"%s_offset"%self.IKFootCtr)
    ...
    def createFootIKHandle(self,startJoint,endJoint):
        ...
        self.footIKNull = self.createGP(self.footIKHandle[0],"ConnectionFrom__%s"%self.IKFootCtr)
        self.footIKGP = self.createGP(self.footIKNull,"%s_IKHandle_Grp"%endJoint)
    ...
    def createKneeIKCtl(self,IKjoint,CtlColor,CtlScale):
        self.IKKneeCtl = self.createRigController(self.TDcrc.TDsphere,IKjoint,IKjoint,CtlColor,1*CtlScale)
    ...
    def createFootFKIKSwitchCtl(self,footJoints,CtlScale):
        self.FootFKIKSwitchCtl = self.createRigController(self.TDcrc.TDCross,"%sFKIKSwitch"%footJoints[-1],self.FKJoint[0],20,1*CtlScale)
        ...
        self.createHideAttr(self.FootFKIKSwitchCtl[1],"t",1,0)
        self.createHideAttr(self.FootFKIKSwitchCtl[1],"r",1,0)
        self.createHideAttr(self.FootFKIKSwitchCtl[1],"s",1,0)
        self.createHideAttr(self.FootFKIKSwitchCtl[1],"v",1,0)
    ...
    def addFootFKIKSwitchAttr(self,footJoints):
        ...
        self.FootSwitch = self.createStringAddAttr(self.FootFKIKSwitchCtl[1],"FK_IK","float","FK:IK:",1)
    ...
    def createFootFKConnection(self,footJoints):
        for Joints in range(len(footJoints)):
            self.matrixConstraint(self.FKFootCtls[Joints],self.FKJoint[Joints])
